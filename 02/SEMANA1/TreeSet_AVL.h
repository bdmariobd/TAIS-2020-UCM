...
protected:
   ...
   struct TreeNode {
      ...
      Link iz, dr;
      int altura,tam_i;
      TreeNode(T const& e, Link i = nullptr, Link d = nullptr, int alt = 1, int tamI = 1) : elem(e), iz(i), dr(d), altura(alt) ,tam_i(tamI) {}
   };
...
public:
   ...
   T const& kesimo(int k) const {
       ...
       while (num != punt->tam_i) {
           if (punt->tam_i > num) {  //tengo que visitar el hijo izquierdo
               punt = punt->iz;
           }
           else if (punt->tam_i < num) { //hijo derecho
               num = num - punt->tam_i;
               if (punt->dr == nullptr) throw std::out_of_range("??");
               punt = punt->dr;
...
protected:
   ...
   bool inserta(T const& e, Link& a) {
       ...
       if (a == nullptr) { // se inserta el nuevo elemento e
           ...
           // En el caso de insertar por la izquierda
           // 1. Compruebo si el árbol crece por la izquierda, si lo hace, aumento el tamaño del nodo padre y de la raiz
           // 2. Si crece, compruebo el re-equilibrar para ver si los tamaños cambiarán.
       ...
       else if (menor(a->elem, e)) { //inserta por la derecha
   ...
   void rotaDer(Link& r2) {
       ...
       r2->tam_i -= r1->tam_i;
       r1->dr = r2;
       // Si rota hacia la derecha, perderá un nodo R2 por la izquierda.
       /*r1->tam_i++;
       raiz->tam_i--;
       r2->tam_i--;*/
   ...
   void rotaIzq(Link& r1) {
       ...
       r2->tam_i += r1->tam_i;
       //else r2->tam_i = 1;
       // Si rota hacia la izquierda, perderá un nodo R2 por la derecha
       /*r1->tam_i--;
       r2->tam_i++;
       raiz->tam_i++;*/
       r1->altura = std::max(altura(r1->iz), altura(r1->dr)) + 1;
